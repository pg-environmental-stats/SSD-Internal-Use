...
fitBV.PLL <- function(
  ...
  fBasedCritVal <<- qf(ConfLevel,1,nrow(BVdata)-1)
  ...
  if(FALSE){# no longer needed
    if(all(groupSizes>1)){
      ...
      if(all(williamsP>0.05) & trendP>0.05){
        ...
        if(nchar(msgText)>0 & doplots)mtext(side=1,line=-2.5,outer=TRUE,text="No evidence of trend in data",cex=.5,adj=0.98)
        if(nchar(msgText)==0 & doplots){
